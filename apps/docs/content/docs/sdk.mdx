...
You will need to add a retry flag in the function and set it to `false` initially
```typescript
async getUserAddresses(retry = false): Promise<CallToolResult>
```

Then in the catch block of this function, add this code
...
try {
    const refreshedToken = await this.authManager.refreshToken("osiris", token.refresh_token);
    ...
    if (!refreshedToken) {
        throw createErrorResponse(new Error("Failed to get user addresses, and failed to refresh token, reauthenticate with this MCP"));
    }
...
This will try to refresh the token and retry the function, to prevent retrying the function unlimited amount of times, we add the retry flag, if it failed then it will throw an error
