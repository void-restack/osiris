...
Here’s how to implement a tool that authenticates with the user's wallet to fetch their available addresses. This is the first step in creating a wallet-aware session.
...
const HYPERLIQUID_API = '[https://api.hyperliquid.xyz](https://api.hyperliquid.xyz)';
    ...
    try {
        ...
        await createMcpServer({
            ...
            auth: {
                ...
                database: new PostgresDatabaseAdapter({ connectionString: databaseUrl }),
            },
            ...
            server: {
                ...
                baseUrl: process.env.HYPERLIQUID_MCP_BASE_URL || `http://localhost:${port}`,
...
```
  ...
  npx @modelcontextprotocol/inspector --config config.json node build/index.js --debug
  ```
  ...
  # Call a tool
  echo '{"jsonrpc":"2.0","method":"tools/call","params":{"name":"your_tool","arguments":{"param":"value"}},"id":2}' | node build/index.js | jq
  ```
  ...
  Always log to `stderr`, never `stdout` - stdout is reserved for JSON-RPC messages only.
  </Callout>
