...
export function McpSimilarCard(props: McpCardProps) {
  return (
    <div className="flex gap-3 items-center border shadow p-3 rounded-[12px] w-full max-w-md">
      <McpCardIcon icon={props.icon} />
      <div className="flex flex-col gap-1">
        <div className="flex items-center gap-3">
          <h2 className="font-medium text-[#171717]">{props.title}</h2>
          {props.isVerified && <ICONS.verifiedBadge className="size-[14px] shrink-0" />}
        </div>
        <p className="text-sm text-[#A3A3A3]">@{props.userHandle}</p>
      </div>
    </div>
  );
}
