...
export function SmartOAuthConnector({
    ...
    const handleConnect = useCallback(async () => {
        ...
        try {
            ...
            const urlData = await createServiceConnection.mutateAsync({
                name: connectionName,
                serviceClientName: serviceName,
                scopes: selectedScopes.map(scope => scope.id),
                redirectUri: `${window.location.origin}/_hub/auth/?success=true&popup=true&type=oauth&serviceClient=${serviceName}`,
            });
            ...
            const handleMessage = (event: MessageEvent) => {
                ...
                if (event.data?.type === 'oauth-success') {
                    setStatus('success');
                    setNewConnectionId(event.data.connectionId);
                    onConnectionSuccess?.();
                    ...
                    toast.error(`Failed to connect ${serviceName}: ${event.data.error || 'Unknown error'}`);
    ...
    }, [selectedScopes, connectionName, serviceName, createServiceConnection, startPolling, onConnectionSuccess]);
