...
export function KnowledgeBaseTable({
        ...
        () => [
            ...
            },
            {
                ...
                cell: ({ row }) => {
                    const kb = row.original;
                    return (
                        <p className="text-xs text-primary-400 max-w-[200px] w-full truncate">{kb.description}</p>
                    );
                },
            },
            {
    ...
    const activeColumns = useMemo(() => {
        const selectedColumns = viewMode === "grid" ? KnowledgeBaseGridViewColumns : columns;
        const searchColumn = selectedColumns.find(col => col.id === "search");
        return selectedColumns;
    }, [viewMode, columns]);

    const { table, data, pagination, isLoading, error, isFetching } = useKnowledgeBaseTable({
