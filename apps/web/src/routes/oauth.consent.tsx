...
function RouteComponent() {
  ...
  const { data: packageDeployments } = useSuspenseQuery(packageQueries.userDeploymentsForPackageOptions(package_id as string))
  const existingPackageDeployments = packageDeployments?.data || []
  ...
  const hasAllRequiredScopes = useCallback((connection: any, requiredScopes: string[], serviceName: string) => {
    ...
    return requiredScopesArray.every(requiredScope => {
      // Check various scope formats - the requiredScope should already be in the correct format
      // from the serviceClientMap, so we just need to check if it exists in connectionScopes
          ...
          {Object.entries(authScopes?.serviceClientMap || {}).map(([serviceName, requiredScopes]) => {
            ...
            const permissions = useMemo(() => {
              ...
              const filteredScopeDefinitions = Object.entries(scopeDefinitions).filter(([scope]) => {
                // The requiredScopes from serviceClientMap should already be in the correct format
                // so we just need to check if the scope exists in the required scopes
                                                    ...
                                                    >
                                                      <div>
                                                        <p className="text-sm font-medium text-primary-800">
                                                          {permission?.label || displayName}
                                                        </p>
                                                        <p className="text-xs text-primary-500">{displayName}</p>
                                                      </div>
                                                      <CheckIcon className="h-4 w-4 text-success-600" />
                                                    </div>
                                                  )
